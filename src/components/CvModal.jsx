...
export default function CvModal({ isOpen, onClose }) {
  ...
  return (
    ...
    >
      ...
      {/* Modal panel */}
      <div
        className={`relative w-full max-w-4xl bg-slate-900 border border-slate-700/60 sm:rounded-2xl overflow-hidden shadow-[0_25px_80px_rgba(0,0,0,0.8)] transition-all duration-300 ${
          isOpen ? 'scale-100 opacity-100' : 'scale-95 opacity-0'
        }`}
        style={{ height: 'min(100dvh, 800px)' }}
      >
        {/* Header */}
        <div className="flex items-center justify-between px-4 sm:px-6 py-3 border-b border-slate-700/50 bg-slate-900/80 backdrop-blur-sm">
          ...
          <div className="flex items-center gap-1.5">
            ...
            {/* Open in new tab (visible on all sizes as fallback) */}
            <a
              href={personalInfo.cvPdf}
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center gap-1.5 px-3 py-2 bg-slate-700 hover:bg-slate-600 text-white rounded-lg text-xs font-semibold transition-all duration-200 sm:hidden"
            >
              פתח
            </a>
